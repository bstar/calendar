...
```typescript
{
  ...
  enabled: true,
  // Optional overall operating window (if needed)
  // minDate / maxDate are supported here, but use boundary rules elsewhere
  minDate: '2025-01-01',
  maxDate: '2025-12-31',
  ranges: [{
...
**Performance Note**: When using `restricted_boundary` with many ranges, be aware that each day cell checks all boundaries during render. For optimal performance, limit the number of ranges to those actually visible or needed. If you need many boundaries (e.g., every month for a year), consider generating ranges dynamically based on the visible months or implementing pagination.
